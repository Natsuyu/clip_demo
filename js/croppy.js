(function() {
    ...
    function ajax(url, data) {
        ...
        else if (typeof ActiveObjectXML != "undefined") xhr = new ActiveObjectXML(arguments.callee.activeXString)
    ...
    var Croppy = (function() {
            ...
            function Croppy(item, option, cb) {
                var that = this
                ...
                if (option.size) this.setting.rulerSize = option.size
                if (option.sessionId) this.id = option.sessionId
            ...
            Croppy.prototype = {
                inithtml: function() {
                    ...
                    tmp.innerHTML = '<div class="cover"><div class="container"><h3 class="tip">缩放图片并移动田字格，使得单个汉字撑满田字格，或单个数字/字幕撑满半个田字格</h3><div class="layer-content"><div class="ruler"><div class="ruler-block"></div><div class="ruler-block"></div><div class="ruler-block"></div><div class="ruler-block"></div></div><canvas class="canvas"></canvas><canvas class="canvas-cover"></canvas></div><div class="toolbar"><div class="tools btn clip" active="true">裁剪</div><div class="tools btn earase">擦除</div><div class="tools btn match">文字匹配</div><div class="tools" id="rotate"><div class=" btn clock">顺时针旋转</div><input class="deg" type="text" placeholder="1"><div class="btn unclock">逆时针旋转</div></div><div class="tools btn oto">1:1</div><div class="tools"><div class="btn preStep">上一步</div><div class="btn nextStep">下一步</div></div><div class="tools"><div class="btn reset">重置</div><div class="btn close">关闭</div></div></div><div class="toolbar"><div class="btn sure">确认</div></div></div></div>'
                    this.father.appendChild(tmp)
                },
                init: function() {
                    console.log("init")
                    this.cover = this.father.$$(this.setting.selector.cover)[0]
                    ...
                    this.cover.style.cssText = "display:block"
                ...
                _close: function() {
                    ...
                    tmpctx.restore()
                    var src = tmpcan.toDataURL("image/png"),
                        // $$("#test")[0].src = tmpcan.toDataURL("image/png")
                        url = 'http://www.zhicuo.cn/interface/testdemo'
                    if (this.callback) this.callback(this.id)
                    else {
                        // var forms = document.createElement("div")
                        // forms.innerHTML = '<form name="form1" method="post" enctype="multipart/form-data" action="http://www.zhicuo.cn/interface/testdemo"><input type="text" name="zhicuoimage"/><input type="text" name="width" /><input type="text" name="height" /><input type="text" name="sendid" /><input type="submit" name="Submit" value="提交" /></form>'
                        // document.body.appendChild(forms)
                        //     // var form = $(forms).find("form")
                        // var form = forms.$$("form")[0]
                        // form["zhicuoimage"].value = src
                        // form["width"].value = width
                        // form["height"].value = height
                        // form["sendid"].value = id
                        ajax(url, "zhicuoimage=" + src + "&width=" + width + "height=" + height + "&sendid=" + id)
                        this.ele.src = src
                        this.cover.style.cssText = "display:none;"
        ...
        //option: Object
        //callback: Function
    Object.prototype.Croppy = function(option, cb) {
        return this.forEach(function(item) {
            var data = item.data

            if (!data) item.data = new Croppy(item, option, cb)
